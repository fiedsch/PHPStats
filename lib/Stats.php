...
class Stats {
	 ...
	 */
	/*
	  // Original definition that causes wrong results
	...
	*/
	// Test using Code (without permission, so not ready for deployment!)
	// from 
	// http://commons.apache.org/proper/commons-math/apidocs/org/apache/commons/math3/special/Erf.html
	// http://svn.apache.org/repos/asf/commons/proper/math/trunk/src/main/java/org/apache/commons/math3/special/Erf.java
	...
	// $alpha = 0.05;
	// 
	// print \PHPStats\ProbabilityDistribution\Normal::getPpf(1 - $alpha/2) ."\n";
	// 
	// $df = 10000;
	// print \PHPStats\ProbabilityDistribution\StudentsT::getPpf(1 - $alpha/2, $df) ."\n";
	//
	// yields
	// 
	...
	public static function ierf($x) {
	  ...
	  $p = (float)PHP_INT_MAX; // faslch, aber für ersten Test ...
	  //$p = (float) INF; // besser?
        }
